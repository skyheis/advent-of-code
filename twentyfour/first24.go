...
func init() {
	TwentyfourCmd.AddCommand(firstCmd)
}

func dayOneGetSortedSlices(scanner *bufio.Scanner) (leftList []int, rightList []int) {
	var left, right int
	var err error

	for scanner.Scan() {
		line := scanner.Text()
		slitted := strings.Fields(line)

		left, err = strconv.Atoi(slitted[0])
		utils.CheckError(err)
		right, err = strconv.Atoi(slitted[1])
		utils.CheckError(err)
...
func dayOneSecondPart(scanner *bufio.Scanner) (result int) {

	leftList, rightList := dayOneGetSortedSlices(scanner)

	i := 0

	for _, val := range leftList {
		valTimes := 0
		for _, comp := range rightList[i:] {
			if comp > val {
				break
			}
			if comp == val {
...
func dayOneFirstPart(scanner *bufio.Scanner) (result int) {
