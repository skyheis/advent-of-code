...
Here the problem statement: https://adventofcode.com/2024/day/5`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		inputPath := "twentyfour/inputs/five.input"
...
func createRulesMap(scanner *bufio.Scanner) map[int][]int {
	var rules = make(map[int][]int)
	var err error

	scanner.Scan()
	for strings.Compare(scanner.Text(), "") != 0 {
		var tmpArr [2]int
		combo := strings.Split(scanner.Text(), "|")
		tmpArr[0], err = strconv.Atoi(combo[0])
		utils.CheckError(err)
		tmpArr[1], err = strconv.Atoi(combo[1])
		utils.CheckError(err)

		rules[tmpArr[0]] = append(rules[tmpArr[0]], tmpArr[1])
...
func dayFiveSecondPart(scanner *bufio.Scanner) (result int) {
	rules := createRulesMap(scanner)

	for scanner.Scan() {
		pagesStr := strings.Split(scanner.Text(), ",")
		pages := utils.SliceAtoi(pagesStr)

		safe := true

		// for i, val := range pages {
		for x := 0; x < len(pages); x++ {
			rule, ok := rules[pages[x]]
			current := x
			for i := 0; i < current && ok; i++ {
				if slices.Contains(rule, pages[i]) {
					pages = utils.SlicesMoveFront(pages, current, i)
					current--
					safe = false
					x = 0
				}
...
func dayFiveFirstPart(scanner *bufio.Scanner) (result int) {
	rules := createRulesMap(scanner)
