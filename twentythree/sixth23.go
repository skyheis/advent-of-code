...
var sixthCmd = &cobra.Command{
	...
	Long: `Day 6 of Advent of Code 2023, my very first Advent of Code!!!
Here the problem statement: https://adventofcode.com/2023/day/6`,
	Run: func(cmd *cobra.Command, args []string) {
		fmt.Printf("Executing %s\n\n", cmd.Short)

		inputPath := "twentythree/inputs/six.input"
		filecontent, err := os.ReadFile(inputPath)
		utils.CheckInputFileError(err)

		start := time.Now()
		result := daySixFirstPart(filecontent)
		...
		start = time.Now()
		result = daySixSecondPart(filecontent)
		end = time.Since(start)
...
func init() {
	TwentythreeCmd.AddCommand(sixthCmd)
}

func daySixFirstPartGetTimes(m []string) ([]int, []int) {
	var e error
...
func daySixFirstPart(filecontent []byte) (result int) {

	result = 1
	raceTime, distance := daySixFirstPartGetTimes(strings.Split(string(filecontent), "\n"))
...
func daySixSecondPartGetTimes(m []string) (int, int) {
	var e error
...
func daySixSecondPart(filecontent []byte) (result int) {

	raceTime, distance := daySixSecondPartGetTimes(strings.Split(string(filecontent), "\n"))
